import flask
from forms import LoginForm, RegistrationForm, PostForm, Feedback
...
class Fedback(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    feed = db.Column(db.String(300), nullable=False)
            ...
            if bcrypt.check_password_hash(user.password, password=password):
                ...
                try:
                    if user is not None:
                        ...
                        def main_page():
                            return flask.render_template("home.html", chat=flask.url_for("chat"))
...
def feedback():
    form = Feedback()
    home = flask.url_for("main_page")
    if form.validate_on_submit():
        back = Fedback(feed=form.feedback.data)
        db.session.add(back)
        db.session.commit()
